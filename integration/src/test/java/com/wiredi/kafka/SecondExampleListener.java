...
@Wire(proxy = false)
public class SecondExampleListener implements KafkaListener {
    ...
    public void prime(int successAfter) {
        this.successAfter = successAfter;
        this.counter = 0;
        this.result.prime(successAfter);
    }
    ...
    public void handle(@NotNull ListenerEvent event) {
        ...
        if (counter < successAfter) {
            result.add(new Handled(body, ERROR));
            throw ERROR;
        } else {
            ConsumerRecord<byte[], byte[]> originalRecord = event.details().record();
            ...
            result.add(new Handled(body, null));
        }
    }

    @Override
    public @NotNull Topics topics() {
        return Topics.of("test2");
    }
